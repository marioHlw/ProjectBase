...
public class MediaPlayerCtrl : MonoBehaviour
{
        ...
        if (m_TargetMaterial != null)
        {
            ...
            for (int iIndex = 0; iIndex < m_TargetMaterial.Length; iIndex++)
            {
                if (m_TargetMaterial[iIndex] != null)
                {
                    ...
                    //if (m_TargetMaterial[iIndex].GetComponent<UITexture>() != null)
                    //{
                    //    m_TargetMaterial[iIndex].GetComponent<UITexture>().uvRect = new Rect(0, 1, 1, -1);
                    //}
	...
	Call_Load(strName,0);
        ...
        if (m_CurrentState == MEDIAPLAYER_STATE.PLAYING || m_CurrentState == MEDIAPLAYER_STATE.PAUSED)
        {
            if (m_bCheckFBO == false)
            {
				...
				{
                    int width = Call_GetVideoWidth ();
                    int height = Call_GetVideoHeight ();

                    if(width < 0 || height < 0) return;

                    m_VideoTextureY = new Texture2D(width, height, TextureFormat.Alpha8, false);
                    m_VideoTextureU = new Texture2D(width / 2, height / 2, TextureFormat.Alpha8, false);
                    m_VideoTextureV = new Texture2D(width / 2, height / 2, TextureFormat.Alpha8, false);
				...
				else
				{
					...
					if (m_bSupportRockchip) {
                        m_VideoTexture = new Texture2D(width, height, TextureFormat.RGB565, false);
	...
	if(m_TargetMaterial[i].GetComponent<UITexture>()!= null)
	{
	if(m_TargetMaterial[i].GetComponent<UITexture>().mainTexture != m_VideoTexture)
	{
	m_TargetMaterial[i].GetComponent<UITexture>().mainTexture = m_VideoTexture;
	}
	}
	...
	private void Call_Play(int iSeek)
	{
        ...
        {
            Debug.LogWarning("ÊÓÆµµÄexception");
        }
	}
	...
	{
       try{
          int width = GetJavaObject().Call<int>("GetVideoWidth");
          return width;
       }
       catch(AndroidJavaException e)
       {
	...
	{
        try{
          int height = GetJavaObject().Call<int>("GetVideoHeight");
          return height;
       }
          catch(AndroidJavaException e)
       {
	...
	{
	if(m_TargetMaterial[i].GetComponent<UITexture>() != null)
	m_TargetMaterial[i].GetComponent<UITexture>().mainTexture= _videoTexture;
	}
	}
                ...
                if (m_TargetMaterial != null)
                {
                    for (int i = 0; i < m_TargetMaterial.Length; i++)
                    {
                        ...
                        if (m_bPC_FastMode == true)
                        {
                            ...
                            if (m_TargetMaterial[i].GetComponent<UITexture>() != null)
                            {
                                m_TargetMaterial[i].GetComponent<UITexture>().material = material;

                                if (m_TargetMaterial[i].GetComponent<UITexture>().material.GetTexture("_Y") != m_VideoTextureY)
                                {
                                    m_TargetMaterial[i].GetComponent<UITexture>().material.SetTexture("_Y", m_VideoTextureY);
                                }

                                if (m_TargetMaterial[i].GetComponent<UITexture>().material.GetTexture("_U") != m_VideoTextureU)
                                {
                                    m_TargetMaterial[i].GetComponent<UITexture>().material.SetTexture("_U", m_VideoTextureU);
                                }

                                if (m_TargetMaterial[i].GetComponent<UITexture>().material.GetTexture("_V") != m_VideoTextureV)
                                {
                                    m_TargetMaterial[i].GetComponent<UITexture>().material.SetTexture("_V", m_VideoTextureV);
                                }
                            }
